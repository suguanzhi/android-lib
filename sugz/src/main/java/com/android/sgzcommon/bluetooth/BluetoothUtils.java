...
public class BluetoothUtils {
    ...
    public static BluetoothDevice hasBlueConnect(BluetoothAdapter mBtAdapter) {
        Set<BluetoothDevice> pairedDevices = mBtAdapter.getBondedDevices();
        if (pairedDevices.size() > 0) {
            for (BluetoothDevice device : pairedDevices) {
                boolean isConnected = isConnect(device);
                if (isConnected) {
                    return device;
                }
            }
    ...
    static public boolean isConnect(BluetoothDevice device) {
    ...
    static public boolean setPin(Class<? extends BluetoothDevice> btClass, BluetoothDevice btDevice, String str) {
        try {
